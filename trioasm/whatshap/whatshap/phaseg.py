...
def haplotag2(pred_superreads, read_set, components, ap, locus_file, iteration, f):
    phases = []
...
def run_phaseg(read_list_filename, locus_file, gam_file, vg_file, canu_alignments, pred_haplotigs):
	...
	with ExitStack() as stack:
		...
		all_reads, alleles_per_pos, locus_branch_mapping, readset_all = vg_reader(locus_file, gam_file)
		#total_readsets[0] = all_reads
		total_readsets[0] = readset_all
	...
	for sample, sample_superreads in zip([0], superreads_list):
		...
		components[sample] = overall_components
		haplotag2(sample_superreads, total_readsets[sample], components[sample], accessible_positions, locus_file, 1, f)
