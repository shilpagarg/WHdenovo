...
if args.sub == 'simulate':
	if args.simusub == 'illumina':
		subprocess.call('src/simulate.py illumina %s %s %s'%(args.fasta, args.het, args. dir), shell = True)
	elif args.simusub == 'pacbio':
		subprocess.call('src/simulate.py pacbio %s %s %s %s'%(args.fastq, args.coverage, args.dir, ' '.join(args.fasta)), shell = True)#[0], args.fasta[1], args.fasta[2], args.fasta[3], args.fasta[4], args.fasta[5]), shell = True)
if args.sub == 'validate':
	subprocess.call('src/validate.py %s %s'%(args.outputDir, args.pacbio), shell = True)
if args.sub == 'assemble':
	if args.ped == None:
		subprocess.call('src/assemble.py --illumina1 %s --illumina2 %s --pacbio %s'%(' '.join(args.illumina1), ' '.join(args.illumina2), args.pacbio[0]), shell = True)
	elif args.ped != None:
		print(1,args.pacbio)
		subprocess.call('src/assemble.py --illumina1 %s --illumina2 %s --pacbio %s --ped %s'%(' '.join(args.illumina1), ' '.join(args.illumina2), ' '.join(args.pacbio), args.ped), shell = True)
