...
# PACBIO part
	...
	print('Above are all you need.')
        ...
        '''
        pbsim --seed 10 --prefix mom1 --depth 15 --length-min 12000 --length-max 13000 --accuracy-min 0.99 --sample-fastq ../test/analysis-PGP1_1
6.5_Elf4-5pM-42165-m54336_190222_142322.Q20.fastq  --accuracy-max 1 mom1.het1.cov30.fasta > pbsim.log 2>&1
        '''
        for i in range(6):
                a = subprocess.call('pbsim --seed 10 --prefix %s/%s --depth 20 --sample-fastq %s %s/%s > %s/pbsim.log 2>&1'%(pbccsOutput, family[i], fq, pbccsOutput, FAs[i], pbccsOutput), shell = True
)
